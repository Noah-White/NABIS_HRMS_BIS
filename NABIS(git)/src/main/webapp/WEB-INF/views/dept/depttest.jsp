...
<div id="wrapOriginTree" class="p-3">
	<div class="ms-auto d-flex">
		<h5 id="offcanvasEndLabel1" class="offcanvas-title">전체 구성원</h5>
		<i class='bx bx-sm bxs-folder-open' onclick="openOAll()"></i>
		<i class='bx bx-sm bxs-folder' onclick="closeOAll()"></i>
	</div>
	<div id="originTree">
    ...
    <div
      class="offcanvas offcanvas-end sideTop
      tabindex="-1"
      id="offcanvasEnd"
    ...
    >
      ...
      <div class="offcanvas-header border-bottom">
        <h5 id="offcanvasEndLabel" class="offcanvas-title">조직도 설정</h5>
        <button 
          type="button"
...
<!-- 	        </form> -->
	...
	<div class="offcanvas-body my-auto mx-0">
		<div class="d-flex ">
			<h5 id="offcanvasEndLabel2" class="offcanvas-title">전체</h5>
			<div class="ms-auto">
				<i class='bx bx-sm bxs-folder-open' onclick="openAll()"></i>
				<i class='bx bx-sm bxs-folder' onclick="closeAll()"></i>
			</div>
		</div>
...
$(document).ready(function() { 
	...
	var $selONode;	//원본트리 선택 노드
	var $selNode;	//수정트리 선택 노드
	var leftData;	//삭제할 노드
	...
	$("#btn_allOpen").on('click', function () { openAll(); })
	$("#btn_allClose").on('click', function () { closeAll(); })
...
$('.offcanvas').on('show.bs.offcanvas', function() {
    ...
    //formT 초기화
	data = [];
	selectDeptList();	//ajax deptList최신화
    console.log("selectDeptList()호출 후 deptList : ",deptList);
...
$tree.on('select_node.jstree', function(e,data){
	...
	$selONode = $tree.jstree("get_selected", true);

	console.log("id", $selONode[0].id )
	console.log("text", $selONode[0].text )
	console.log("parent", $selONode[0].parent )
	treeDcode = $selONode[0].id;
 });
...
//deptList 최신화
selectDeptList = function(){
	console.log("selectDeptList ajax호출")
	$.ajax({
		url : CONTEXTPATH+"/dept/deptList.do",
		method : "GET",
		dataType : "json",
		async: false,
		success : function(resp) {
...
totalNode = function(){
	...
	if($formT.jstree(true)){
		...
		$("#offcanvasEndLabel1").text("전체 조직 ("+ totalNum + ")");
      ...
      //추가된 부분 시작
        "data":[
			<c:forEach items="${deptList }" var="dept" >
			{
        		"text" : "${dept.dnm}",
        		"id" : "${dept.dcode}",
        		<c:if test="${empty dept.uprDcode}">
        			"parent" : "#"
        		</c:if>
        		<c:if test="${not empty dept.uprDcode}">
        			"parent" : "${dept.uprDcode}"
        		</c:if>
...
function formTree(){
	...
	$('#formTree').jstree({
	    ...
	    "plugins": ["types","dnd","contextmenu","search"],
	    "search":{
