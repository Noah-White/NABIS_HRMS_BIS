...
<h5 id="offcanvasEndLabel1" class="offcanvas-title">직급</h5>
<div id="originTree">
    ...
    >
      ...
      <div class="offcanvas-header border-bottom">
        <h5 id="offcanvasEndLabel" class="offcanvas-title">직급 설정</h5>
        <button 
          type="button"
...
$('.offcanvas').on('show.bs.offcanvas', function() {
    ...
    //formT 초기화
	data = [];
	selectGrdList();	//ajax grdList최신화
    console.log("selectGrdList()호출 후 grdList : ",grdList);
    console.log("열었을때 oldData", oldData)
	
  	$(grdList).each(function(index, grd){
  		object ={
    		"text" : grd.grdNm,
    		"id" : grd.grdCode,
    		"parent" : "#"
...
//grdList 최신화
selectGrdList = function(){
	console.log("selectDtyList ajax호출")
	$.ajax({
		url : "${cPath}/grd/grdList.do",
		method : "GET",
		dataType : "json",
		async: false,
		success : function(resp) {
...
totalNode = function(){
	...
	if($formT.jstree(true)){
		...
		$("#offcanvasEndLabel1").text("전체 직급 ("+ totalNum + ")");
