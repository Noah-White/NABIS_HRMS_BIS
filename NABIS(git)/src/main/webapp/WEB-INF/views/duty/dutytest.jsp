...
<h5 id="offcanvasEndLabel1" class="offcanvas-title">직책</h5>
<div id="originTree">
    ...
    >
      ...
      <div class="offcanvas-header border-bottom">
        <h5 id="offcanvasEndLabel" class="offcanvas-title">직책 설정</h5>
        <button 
          type="button"
...
$('.offcanvas').on('show.bs.offcanvas', function() {
    ...
    //formT 초기화
	data = [];
	selectDutyList();	//ajax dutyList최신화
    console.log("selectDutyList()호출 후 dutyList : ",dutyList);
...
var setId = function(){
	...
	//db조회
	$.ajax({
		url : "${cPath }/duty/maxDtcode.do",
		method : "GET",
		dataType : "json",
		async: false,
		success : function(resp) {
			console.log("타입",typeof(resp))
			dbNum = Number(resp)
...
//dutyList 최신화
selectDutyList = function(){
	console.log("selectDtyList ajax호출")
	$.ajax({
		url : "${cPath}/duty/dutyList.do",
		method : "GET",
		dataType : "json",
		async: false,
		success : function(resp) {
...
totalNode = function(){
	...
	if($formT.jstree(true)){
		...
		$("#offcanvasEndLabel1").text("전체 직책 ("+ totalNum + ")");
